...
module.exports = {
    ...
    loginAdmin(req, res) {
        doadminLoged(req.body).then((response) => {
            ...
            res.render('admin/admin-login', { error: 'Invalid login details', layout: 'admin-layout' })
        })
    },
    ...
    addProducts(req, res) {
        productHelpers.getCategoryDropdown().then((categoryDropdown) => {
            res.render('admin/add-products', { layout: 'admin-layout', admin: true, categoryDropdown })
        })
    ...
    addProductsSubmit(req, res) {
        productHelpers.addProduct(req.body).then((response) => {
            ...
            image.mv("./public/product-images/" + response + '.jpg', (err) => {
                if (!err) {
                    res.render('admin/add-products', { layout: 'admin-layout', admin: true })
                } else {
                    console.log(err);
                }
            })
    ...
    editProductAction: (async (req, res) => {
        ...
        productHelpers.getCategoryDropdown().then((categoryDropdown) => {
            res.render('admin/edit-product', { product, categoryDropdown, layout: 'admin-layout', admin: true })
        })
    ...
    allCategory(req, res) {
        productHelpers.getAllCategorys().then((categorys) => {
            res.render('admin/all-category', { categorys, layout: 'admin-layout', admin: true });
        })
    ...
    editCategory: (async (req, res) => {
        ...
        let categoryDetails = await productHelpers.getCategoryDetails(req.params.id)
        console.log(categoryDetails + "++++++++++++++++++++++++))))))))))))))))))))))))))))))))))))))))");
        res.render('admin/edit-category', { admin: true, layout: 'admin-layout', categoryDetails })
    ...
    orderManagement : (async(req,res)=>{
        ...
        res.render('admin/order-management', {admin:true ,layout:'admin-layout', orders})
    }),

    viewProductsOrder : (async(req,res)=>{
        let products = await adminHelpers.getProductsOrdermanagement(req.params.id)
        res.render('admin/view-products-order', {admin:true ,layout:'admin-layout', products})
    }),

    cancelOrderManagement : (async(req,res)=>{
        adminHelpers.cancelCurrentOrders(req.params.id,req.body.status).then(()=>{
